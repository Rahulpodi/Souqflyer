-- get_promo_brands_by_category(p_country text, p_category text)
...
CREATE OR REPLACE FUNCTION public.get_promo_brands_by_category(p_country text, p_category text)
 RETURNS TABLE(brand_name text)
 LANGUAGE plpgsql
 STABLE
AS $function$
BEGIN

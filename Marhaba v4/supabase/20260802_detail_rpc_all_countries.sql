-- ============================================================
-- get_flyer_products_detail: allow an empty p_country to mean "all countries"
--
-- Data Export needs a cross-country spreadsheet, but country was the one
-- mandatory predicate. Every other filter already treats blank as "no filter";
-- this brings country in line. Passing a country still scopes exactly as before.
-- ============================================================
...
AS $$
    ...
    FROM (
        ...
        WHERE (nullif(btrim(p_country), '') IS NULL
               OR lower(fp.country) = lower(btrim(p_country)))
